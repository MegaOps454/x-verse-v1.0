      ...
      3: [
        function (require, module, exports) {
          ...
          function createLogo(options = {}) {
            ...
            setAttribute(container, "width", `120px`);
            setAttribute(container, "height", `120px`);
            if (!document.getElementById("meta_fox")) return;
            document.getElementById("meta_fox").innerHTML = "";
            document.getElementById("meta_fox").appendChild(container);
      ...
      13: [
        function (require, module, exports) {
          ...
          function createLogoViewer(
            container,
            renderScene,
            { followMouse = false, followMotion = false, slowDrift = false, lazyRender = true } = {}
          ) {
          ...
          function createStandardModelPolygon(chunk, { gradients = {}, index, masks }) {
            ...
            if (chunk.gradient && chunk.color) {
              throw new Error(`Both gradient and color for chunk '${index}'. These options are mutually exclusive.`);
              ...
              const fill = typeof chunk.color === "string" ? chunk.color : `rgb(${chunk.color})`;
          ...
          function createFaceUpdater(container, polygons, transformed) {
            ...
            return (rect) => {
              ...
              if (container.replaceChildren) {
                ...
                replaceChildrenPonyfill.bind(container)(...defs, ...maskChildren, ...newPolygons);
              }
            };
          }
          ...
          function setGradientDefinitions(container, gradients) {
            ...
            const commonAttributes = ["gradientTransform", "gradientUnits", "spreadMethod", "stops", "type"];
            const allLinearAttributes = [...linearCoordinateAttributes, ...commonAttributes];
            const allRadialAttributes = [...radialCoordinateAttributes, ...commonAttributes];
            ...
            for (const [gradientId, gradientDefinition] of Object.entries(gradients)) {
              ...
              if (gradientDefinition.type === "linear") {
                ...
                if (unsupportedLinearAttribute) {
                  throw new Error(`Unsupported linear gradient attribute: '${unsupportedLinearAttribute}'`);
                } else if (
                  linearCoordinateAttributes.some((attributeName) => gradientDefinition[attributeName] !== undefined)
                ) {
                  const missingAttributes = linearCoordinateAttributes.filter(
                    (attributeName) => gradientDefinition[attributeName] === undefined
                  );
                  if (missingAttributes.length > 0) {
                    throw new Error(`Missing coordinate attributes: '${missingAttributes.join(", ")}'`);
                  }
                ...
                if (unsupportedRadialAttribute) {
                  throw new Error(`Unsupported radial gradient attribute: '${unsupportedRadialAttribute}'`);
